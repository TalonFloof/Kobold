...
pub fn RelocateELF(elf: *anyopaque) ELFError!void {
    ...
    while (i < header.shtEntryCount) : (i += 1) {
        ...
        if (entry.sh_type == 8) {
            entry.addr = @intFromPtr(physmem.Allocate(entry.sh_size, 0));
        } else {
            entry.addr = @intFromPtr(elf) + entry.sh_offset;
        }
    }
    while (i < header.e_shnum) : (i += 1) {
        ...
        const relTable = @as([*]std.elf.Rela, @ptrFromInt(entry.sh_addr))[0..(entry.size / @sizeOf(std.elf.Rela))];
        ...
        for (0..relTable.len) |rela| {
            ...
            const symValue: usize = symTable[sym].value;
            const convAddend: usize = @bitCast(@as(isize, @intCast(relTable[rela].r_addend)));
            switch (builtin.cpu.arch) {
                .riscv64 => {
                    switch (@as(std.elf.R_RISCV, @enumFromInt(@as(u32, @intCast(relTable[rela].info & 0xFF))))) {
                        .@"64" => {
                ...
                .x86_64 => {
                    switch (@as(std.elf.R_X86_64, @enumFromInt(@as(u32, @intCast(relTable[rela].info & 0xFFFFFFFF))))) {
                        .X86_64_64 => {
