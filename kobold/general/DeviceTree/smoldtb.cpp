...
#define SMOLDTB_STATIC_BUFFER_SIZE 16384
...
{
    struct name_collision_check* check = (struct name_collision_check*)opaque;
    const size_t name_len = string_len(node->name);
...
dtb_node* dtb_create_sibling(dtb_node* node, const char* name)
{
    ...
    const size_t name_len = string_len(name);
    char* name_buf = (char*)(state.ops.malloc(name_len + 1));
