// Original Library can be found here: https://github.com/leecannon/zig-sbi/blob/master/sbi.zig
// Copyright (c) 2022-2023 Lee Cannon
...
pub const dbcn = struct {
    ...
    pub fn writeBytes(c: []const u8) error{ NOT_SUPPORTED, INVALID_PARAM, DENIED, FAILED }!isize {
        return try ecall.threeArgsWithReturnWithError(
            .DBCN,
            0,
            @bitCast(c.len),
            @bitCast(@intFromPtr(c.ptr) & 0xFFFF_FFFF),
            @bitCast((@intFromPtr(c.ptr) >> 32) & 0xFFFF_FFFF),
            error{ NOT_SUPPORTED, INVALID_PARAM, DENIED, FAILED },
        );
    }

    pub fn writeByte(c: u8) error{ NOT_SUPPORTED, INVALID_PARAM, DENIED, FAILED }!void {
        try ecall.oneArgs64NoReturnWithError(
            .DBCN,
            2,
            @intCast(c),
            error{ NOT_SUPPORTED, INVALID_PARAM, DENIED, FAILED },
